...
# BeWare Market Bot Health Check Script
echo "🔍 BeWare Market Bot Health Check"
echo "================================="
...
check_process() {
    ...
    if command -v pm2 &> /dev/null; then
        PM2_STATUS=$(pm2 list | grep "beware-market-bot" | grep "online" | wc -l)
        if [ "$PM2_STATUS" -gt 0 ]; then
            echo "✅ PM2 process running"
            pm2 show beware-market-bot | grep -E "(status|uptime|memory|cpu)"
        else
...
BeWare Market Bot Health Check Report
Generated: $TIMESTAMP
...
1. Process not running: npm start or pm2 restart beware-market-bot
